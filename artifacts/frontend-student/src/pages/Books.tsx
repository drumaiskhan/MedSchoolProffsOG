...
function Books() {
  ...
  return <div><SectionHeader eyebrow="Library" title="Books" action={<div className="relative"><Search className="absolute left-3 top-2.5 text-muted-foreground" size={15} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search books" className="h-9 w-44 rounded-xl border border-border bg-card pl-9 pr-3 text-xs outline-none focus:ring-2 focus:ring-primary/20" data-testid="input-search-books" /></div>} />
    {filtered.length ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{filtered.map((b) => { const url = resolveUploadUrl(b.storagePath); const secure = !b.locked && !!b.secureReader; const openable = !b.locked && (secure || url); const Card: ElementType = openable ? (secure ? Link : 'a') : 'div'; return <Card key={b.id} {...(openable ? (secure ? { href: `/books/${b.id}/read` } : { href: url!, target: '_blank', rel: 'noreferrer' }) : {})} className={cn('card-lift rounded-2xl border border-border bg-card p-4', !openable && !b.locked && 'opacity-60')} data-testid={`row-book-${b.id}`}>
      ...
      <div className="flex items-start justify-between gap-2"><h3 className="text-sm font-bold leading-5">{b.title}</h3><span className={cn('shrink-0 rounded-full px-2 py-0.5 text-[10px] font-extrabold', b.isFree ? 'bg-[#d7eee4] text-[#164b4b]' : !b.locked ? 'bg-[#d7eee4] text-[#164b4b]' : 'bg-muted text-muted-foreground')} data-testid={`text-book-tier-${b.id}`}>{b.isFree ? 'Free' : !b.locked ? 'Owned' : b.price != null ? money(b.price, b.currency ?? 'PKR') : 'Paid'}</span></div>
      ...
      {b.locked && !b.purchasePending && buyingId !== b.id && <button onClick={(e) => { e.preventDefault(); setBuyingId(b.id); }} className="mt-2 rounded-lg bg-primary px-3 py-1.5 text-[11px] font-bold text-primary-foreground" data-testid={`button-buy-book-${b.id}`}>Buy access</button>}
