...
function StudentYearGroup({ programLabel, yearLabel, students, onSelect, onDelete }: { programLabel: string; yearLabel: string; students: Student[]; onSelect: (id: number) => void; onDelete: (id: number) => void }) {
  ...
  return <div className="rounded-2xl border border-border bg-card">
    <button type="button" onClick={() => setOpen((v) => !v)} className="flex w-full items-center gap-2 px-5 py-3.5 text-left" data-testid={`button-student-year-group-${programLabel}-${yearLabel}`}>
      ...
      <span className="text-[11px] text-muted-foreground">{students.length} student{students.length === 1 ? '' : 's'}</span>
    </button>
    {open && <div className="space-y-3 border-t border-border p-4">
      <div className="space-y-3 sm:hidden">{students.map((s) => <div key={s.id} onClick={() => onSelect(s.id)} className="cursor-pointer rounded-2xl border border-border bg-background p-4" data-testid={`card-student-${s.id}`}><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><div className="grid size-9 shrink-0 place-items-center rounded-full bg-[#d7eee4] text-[10px] font-bold text-primary">{initials(s.name)}</div><div><div className="text-sm font-bold">{s.name}</div><div className="mt-0.5 text-[11px] text-muted-foreground">{s.email}</div></div></div><button onClick={(e) => { e.stopPropagation(); onDelete(s.id); }} className="shrink-0 rounded-lg p-1.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" data-testid={`button-delete-student-mobile-${s.id}`}><Trash2 size={14} /></button></div><div className="mt-3 flex items-center justify-between text-[11px]"><span className="text-muted-foreground">{s.institution} · {s.program}</span><Badge tone={s.status === 'ACTIVE' ? 'green' : s.status === 'SUSPENDED' || s.status === 'REJECTED' ? 'red' : 'amber'}>{s.status}</Badge></div><div className="mt-3 flex items-center gap-2"><Progress value={s.progress || 0} /><span className="font-mono-app text-[10px] text-muted-foreground">{s.progress || 0}%</span></div></div>)}</div>
      <div className="hidden overflow-x-auto rounded-xl border border-border sm:block"><table className="w-full min-w-[760px] text-left text-xs"><thead className="bg-muted text-[10px] uppercase tracking-[.12em] text-muted-foreground"><tr><th className="px-5 py-3">Student</th><th className="px-5 py-3">Institution</th><th className="px-5 py-3">Status</th><th className="px-5 py-3">Progress</th><th className="px-5 py-3"></th></tr></thead><tbody>{students.map((s) => <tr key={s.id} className="cursor-pointer border-t border-border hover:bg-muted/40" data-testid={`row-student-${s.id}`}><td className="px-5 py-4" onClick={() => onSelect(s.id)}><div className="flex items-center gap-3"><div className="grid size-8 place-items-center rounded-full bg-[#d7eee4] text-[10px] font-bold text-primary">{initials(s.name)}</div><div><div className="font-bold">{s.name}</div><div className="mt-1 text-[10px] text-muted-foreground">{s.email}</div></div></div></td><td className="px-5 py-4" onClick={() => onSelect(s.id)}><div>{s.institution}</div><div className="mt-1 text-[10px] text-muted-foreground">{s.program}</div></td><td className="px-5 py-4" onClick={() => onSelect(s.id)}><Badge tone={s.status === 'ACTIVE' ? 'green' : s.status === 'SUSPENDED' || s.status === 'REJECTED' ? 'red' : 'amber'}>{s.status}</Badge></td><td className="w-40 px-5 py-4" onClick={() => onSelect(s.id)}><div className="flex items-center gap-2"><Progress value={s.progress || 0} /><span className="font-mono-app text-[10px]">{s.progress || 0}%</span></div></td><td className="px-5 py-4 text-right"><button onClick={(e) => { e.stopPropagation(); onDelete(s.id); }} className="rounded-lg p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" data-testid={`button-delete-student-${s.id}`}><Trash2 size={15} /></button></td></tr>)}</tbody></table></div>
    </div>}
  </div>;
}
...
function AdminStudents() {
  ...
  return <div><SectionHeader eyebrow="People operations" title="Students" action={<span className="text-[10px] text-muted-foreground">{students.length} students</span>} /><div className="mb-4 flex flex-wrap items-center gap-3"><div className="relative max-w-sm flex-1"><Search className="absolute left-3 top-2.5 text-muted-foreground" size={15} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search students, institutions..." className="h-10 w-full rounded-xl border border-border bg-card pl-9 pr-3 text-xs outline-none" data-testid="input-search-students" /></div><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="h-10 rounded-xl border border-border bg-card px-3 text-xs" data-testid="select-student-status-filter"><option value="">All statuses</option>{STUDENT_STATUSES.map((s) => <option key={s} value={s}>{s.replace(/_/g, ' ')}</option>)}</select></div>{q.isLoading ? <SkeletonPage /> : students.length ? <div className="space-y-3">
    {groups.map((g) => <StudentYearGroup key={`${g.programLabel}-${g.yearLabel}`} programLabel={g.programLabel} yearLabel={g.yearLabel} students={g.groups.map((x) => x.student)} onSelect={setSelectedId} onDelete={setDeletingId} />)}
  </div> : <EmptyState icon={Users} title="No students yet" body="Students will appear here once they register." />}{selectedId !== null && <StudentDrawer id={selectedId} onClose={() => setSelectedId(null)} />}{deletingId !== null && <ConfirmDialog title="Permanently delete this student?" body="This erases their account, payments, memberships, practice/exam history, and everything else tied to it, for good. There is no undo." confirmLabel="Delete forever" onCancel={() => setDeletingId(null)} onConfirm={() => removeStudent.mutate(deletingId)} pending={removeStudent.isPending} />}</div>;
}
